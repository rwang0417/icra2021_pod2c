...
fid = fopen('./nominal_control/result0_t2d1.txt','r');
U = fscanf(fid, '%f');
fclose(fid);
...
mexstep('load',['./model/',MODEL]); % load model
...
save('arma_fdbk.mat','MTK','MCK','MQ','MQU','MYE','MU1','MYM');

%% closed-loop performance test
vk = .001*randn(OUT_NUM,STEP_NUM+1,TEST_NUM);
...
avg_dz = mean((abs(y_closed)),1)
max_dz = max(abs(y_closed))
